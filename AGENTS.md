...
- `src/components/sections/FAQSection.tsx`: secao `#faq`; perguntas com abertura por hover no desktop e fallback acessivel por foco/click.
...
- O cursor customizado deve manter contraste alto nos dois temas: tons escuros no modo claro e tons claros no modo escuro, sem alterar formato ou comportamento.
- Reveals de scroll devem ser discretos, com conteudo totalmente visivel quando reduced motion estiver ativo.
- A entrada inicial do `BrowserMockup` deve permanecer leve: use uma camada interna `.hero-mockup-entry` para fade e translate vertical pequeno, mantendo `.hero-mockup-shell` reservado para o zoom/`ScrollTrigger` e em escala final desde o primeiro frame; evite slide lateral grande, rotacao ou filtros nessa primeira revelacao para nao causar jank nem diferenca de tamanho no primeiro carregamento.
- O mockup do hero funciona como mascara narrativa: ao scrollar, `ScrollTrigger` pina a hero com `pinSpacing: false`, amplia o browser por uma distancia de um viewport, separa o conteudo interno para esquerda/direita sem blur e revela a secao `#sobre` real em tamanho normal por tras da hero. Durante a timeline, `#sobre` recebe `.is-about-locked` com `position: fixed`, `inset: 0` e `width: 100%` para ficar travada no topo ate o browser sumir; remova esse lock nos callbacks de saida e no cleanup. As superficies do `BrowserMockup` devem virar uma mascara translucida antes de desaparecer, para evitar frames de fundo opaco vazio e reforcar a sensacao de entrar pela janela do browser. A secao `#sobre` usa `about-mask-reveal` como alvo visual do fade/translate controlado pela timeline; nao use margem negativa para compensar o pin e nao duplique `#sobre` dentro do mockup. Mantenha o fallback reduced motion sem pin nem transformacao.
...
- 2026-05-08, comparacao `HEAD..worktree`: a transicao do zoom narrativo passou a travar a secao real `#sobre` como camada fixa no topo durante o `ScrollTrigger`; a timeline revela a About mais cedo e remove a mascara do browser antes do fim do pin, evitando passos vazios antes da rolagem normal da secao.
- 2026-05-08, comparacao `HEAD..worktree`: o `BrowserMockup` ganhou camadas semanticas para brilho, arco e grid interno; a timeline agora revela a About ainda mais cedo e torna fundo, borda e sombra do browser translucidos antes do fade final, reduzindo os frames em que so aparecia o fundo ampliado do mockup.
- 2026-05-08, comparacao `HEAD..worktree`: a entrada inicial do `BrowserMockup` foi separada do zoom narrativo: `.hero-mockup-entry` cuida de `autoAlpha` e translate vertical pequeno, enquanto `.hero-mockup-shell` fica reservado para `ScrollTrigger` em escala final desde o primeiro frame, sem slide lateral nem rotacao; os hints de composicao e testes cobrem essa separacao para reduzir jank no primeiro carregamento.
- 2026-05-08, comparacao `HEAD..worktree`: antes do lock fixo, o zoom narrativo usava `.is-about-locked` com `position: sticky` para tentar preservar o fluxo antes de `#como-funciona`; a separacao interna do browser deixou de usar blur e a timeline foi retimada para revelar o About antes do fade final da mascara.
- 2026-05-06, comparacao `HEAD..worktree`: o reveal da secao `#sobre` deixou de deslocar a section inteira; o fundo agora encaixa no topo do viewport, enquanto apenas `.about-mask-reveal__content` recebe translate leve. A distancia do pin voltou a ser um viewport completo e o fade final da mascara do browser foi atrasado para evitar a faixa indesejada antes do topo da secao.
- 2026-05-06, comparacao `HEAD..worktree`: a timeline do zoom narrativo ficou mais curta e fluida; `ScrollTrigger` usa scrub menor, distancia de reveal reduzida, fade antecipado da secao `#sobre` e fade mais cedo da mascara do browser para evitar o trecho intermediario em que o `BrowserMockup` ocupa a tela como abstracao vazia.
- 2026-05-06, comparacao `HEAD..worktree`: a transicao do hero para `#sobre` deixou de usar margem negativa com pin spacing normal. `ScrollTrigger` agora usa `pinSpacing: false`, a secao real `#sobre` recebe `ref` e fade/translate na timeline, e `about-mask-reveal` permanece apenas como classe de alvo visual para evitar que a primeira tela da secao apareca cortada depois do zoom do `BrowserMockup`.
...
- 2026-05-08, comparacao `HEAD..worktree`: a home ganhou entrada sincronizada do texto do hero com o mockup, CTAs comerciais agora usam hover de escala e troca de cor, FAQ abre por hover com fallback de foco/click, o BrowserMockup acompanha melhor o dark mode e o cursor ganhou contraste por tema.
- 2026-05-08, comparacao `HEAD..worktree`: os CTAs do hero foram padronizados pelo estilo secundario, os cards do `BrowserMockup` ficaram uniformes e mais legiveis no dark mode, os CTAs finais usam hover champagne e o cursor ativa inversao por `mix-blend-mode` apenas em `#contato`.
- 2026-05-08, comparacao `HEAD..worktree`: o dark mode do `BrowserMockup` passou a usar superficies grafite especificas para mask/body/stage/viewport, textos internos com classes semanticas e hover dark explicito nos cards para evitar conflito de especificidade com `group-hover`.
- 2026-05-08, comparacao `HEAD..worktree`: as camadas grandes do `BrowserMockup` deixaram de depender de backgrounds claros no JSX; os fundos light/dark agora vivem em CSS semantico e o hover dos cards no dark mode segue o padrao da `AboutSection`, com superficie clara e texto escuro.
