...
export function BrowserMockup({ className }: BrowserMockupProps) {
  return (
    <article
      ...
      className={[
        "overflow-hidden rounded-[1.375rem] border border-[var(--border)]",
        ...
        .join(" ")}
    >
      <div className="hero-browser-chrome relative h-14 border-b border-[var(--border)] px-6">
        ...
        <div className="hero-browser-address absolute left-1/2 top-1/2 flex h-7 w-[min(16rem,42vw)] -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-full border border-[rgba(201,194,180,0.6)] px-4">
      ...
      <div className="hero-browser-body p-6">
        <div className="hero-browser-stage relative min-h-[19.875rem] overflow-hidden rounded-[1.125rem] border border-[var(--border)] p-[1.125rem]">
          <div
            aria-hidden="true"
            className="hero-browser-glow absolute inset-0"
          />
          <div className="hero-browser-viewport relative min-h-[17.625rem] overflow-hidden rounded-[0.875rem] border border-[rgba(201,194,180,0.45)]">
            <div
              aria-hidden="true"
              className="hero-browser-depth absolute inset-0"
            />
            <div
              aria-hidden="true"
            ...
            <div className="hero-browser-preview-grid relative grid min-h-[17.625rem] gap-5 p-7 pb-16 md:grid-cols-[1.05fr_0.95fr]">
              <div className="hero-browser-left flex flex-col justify-center">
                <p className="hero-browser-preview-eyebrow text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--champagne)]">
                  Próxima seção / sobre
                </p>
                <h2 className="hero-browser-preview-title mt-4 max-w-[18rem] font-serif text-[2.25rem] font-medium leading-[0.98] text-[var(--charcoal)]">
                  Presença digital com aparência de marca confiável.
                </h2>
                <p className="hero-browser-preview-copy mt-5 max-w-[18.5rem] text-xs leading-5 text-[var(--stone)]">
              ...
              <div className="hero-browser-right flex flex-col justify-center gap-3">
                {aboutMetrics.map((metric, index) => (
                  <div
                    key={metric.value}
                    className="hero-browser-metric-card group/card rounded-[0.95rem] border border-[var(--border)] bg-[var(--bone)] px-4 py-3 text-[var(--charcoal)] transition-[background,color,border-color,transform] duration-300 hover:scale-[1.02] hover:border-[var(--charcoal)] hover:bg-[var(--charcoal)] hover:text-[var(--bone)]"
                  >
                    <p className="hero-browser-card-title font-serif text-[1.55rem] font-medium leading-none">
                      {metric.value}
                    </p>
                    <p className="hero-browser-card-copy mt-2 line-clamp-2 text-[10px] leading-4 text-[var(--stone)] transition-colors duration-300 group-hover/card:text-[var(--bone)]">
                      {index === 0
